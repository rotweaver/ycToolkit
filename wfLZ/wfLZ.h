...
DllExport extern uint32_t wfLZ_GetMaxCompressedSize( const uint32_t inSize );

//! wfLZ_GetWorkMemSize()
/*! Returns the minimum size for workMem passed to wfLZ_CompressFast and wfLZ_Compress */
DllExport extern uint32_t wfLZ_GetWorkMemSize();
...
*/
DllExport uint32_t wfLZ_CompressFast( const uint8_t* WF_RESTRICT const in, const uint32_t inSize, uint8_t* WF_RESTRICT const out, const uint8_t* WF_RESTRICT workMem, const uint32_t swapEndian );

//! wfLZ_Compress()
/*! Returns the size of the compressed data
* This is mostly a reference compressor, it is extremely slow.
*/
DllExport extern uint32_t wfLZ_Compress( const uint8_t* WF_RESTRICT const in, const uint32_t inSize, uint8_t* WF_RESTRICT const out, const uint8_t* WF_RESTRICT workMem, const uint32_t swapEndian );

//! wfLZ_GetDecompressedSize()
/*! Returns 0 if the data does not appear to be valid WFLZ */
DllExport extern uint32_t wfLZ_GetDecompressedSize( const uint8_t* const in );

//! wfLZ_GetCompressedSize()
/*! Returns 0 if the data does not appear to be valid WFLZ */
DllExport extern uint32_t wfLZ_GetCompressedSize( const uint8_t* const in );

//! wfLZ_Decompress()
/*! Use wfLZ_GetDecompressedSize to allocate an output buffer of the correct size */
DllExport extern void wfLZ_Decompress( const uint8_t* WF_RESTRICT const in, uint8_t* WF_RESTRICT const out );

//! wfLZ_GetHeaderSize()
/*! Returns 0 if the data does not appear to be valid WFLZ */
DllExport uint32_t wfLZ_GetHeaderSize( const uint8_t* const in );
...
DllExport extern uint32_t wfLZ_GetMaxChunkCompressedSize( const uint32_t inSize, const uint32_t blockSize );
...
*/
DllExport extern uint32_t wfLZ_ChunkCompress( uint8_t* in, const uint32_t inSize, const uint32_t blockSize, uint8_t* out, const uint8_t* workMem, const uint32_t swapEndian, const uint32_t useFastCompress );

//! wfLZ_GetNumChunks()
/*!
* Returns 0 if data appears invalid
*/
DllExport uint32_t wfLZ_GetNumChunks( const uint8_t* const in );
...
*/
DllExport void wfLZ_ChunkDecompressCallback( uint8_t* in, void( *chunkCallback )( void* ) );
...
*/
DllExport uint8_t* wfLZ_ChunkDecompressLoop( uint8_t* in, uint32_t** chunkDesc );
